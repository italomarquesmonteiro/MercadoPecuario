---
title: "Indicador do Boi Gordo Cepea/B3"
description: ""
author script: 
  - name: "Ítalo Marques-Monteiro"
    github: https://github.com/italomarquesmonteiro
    kaggle: https://www.kaggle.com/talomarquesmonteiro
    linkedin: linkedin.com/in/ítalo-monteiro-0b2516144
    lattes: http://lattes.cnpq.br/7174985368517137
    orcid_id: https://orcid.org/0000-0003-4367-1034
    affiliation: Anhanguera College  
        - Department: Veterinary Medicine
date: "2023-08-18"
output: html_document
---
```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```


```{r} 
library(tidyverse)
```

```{r}
readxl::read_xlsx(
  "C:/Users/italo/Downloads/CEPEA_20240103003539.xlsx", skip = 3) |>
  dplyr::mutate(Data = lubridate::dmy(Data)) |>
  janitor::clean_names("snake") |>
  readr::write_csv("Dataset index/CepeaBoiGordo.csv")
```


```{r}
indicador_boi <- readr::read_csv("Dataset index/CepeaBoiGordo.csv") |>
  janitor::clean_names("snake") |>
  dplyr::glimpse()
```

```{r}
boi_sp <- indicador_boi |>
  dplyr::select(-a_vista_us) |>
  dplyr::filter(data >= "2020-10-27")
```


```{r}
scaleFUN <- function(x) format(round(as.numeric(x), 0), nsmall= 0, big.mark = ",")

# Create line chart theme
theme_swd_line <-   theme(
  plot.title = element_text(size = rel(1.75), color = "steelblue"), #steelblue
  plot.margin = margin(20, 20, 20, 20),
  text = element_text(family = "Roboto", color = "#979797"),
  axis.title.x = element_text(hjust = 0, vjust = -0.5),
  axis.title.y = element_blank(),
  axis.text.y = element_blank(),
  line = element_blank(),
  rect = element_blank()
)
```

```{r}
indicador_boi |>
  dplyr::filter(data >= "2023-08-25") |>
  dplyr::arrange(desc(a_vista_r))
```


```{r}
indicador_boi |>
  #dplyr::filter(data >= "2023-08-01") |>
  ggplot2::ggplot(aes(x = data, y = a_vista_r)) +
              geom_smooth(method = "gam", se = FALSE) +
              geom_line(color = "steelblue", size = 1) +
              geom_col(alpha = 0.9) +
              scale_x_date(date_breaks = "1 day", date_labels = "%Y-%m-%d") +
              #scale_y_continuous(limits = c(195, 250, 1)) +
              theme_swd_line +
              labs(
                title = "Indicador @ do Boi Gordo",
                subtitle = "# Fonte: CEPEA/B3(2023)

                by Ítalo Monteiro",
                x = "Data"
              ) +
              theme(axis.text.x = element_text(size = 10, color = "black",
                angle = 60, hjust = 1, vjust = 1)) +
              geom_text(
                    data = indicador_boi[indicador_boi$data == "2023-09-29", ],
                            aes(label = scaleFUN(a_vista_r)),
                            hjust = -0.6,
                            vjust = -0.5,
                            size = 5,
                            color = "steelblue",
                            fontface = "bold") +
                annotate("text", label = "R$/@",
                    x = as.Date(
                        "2023-08-25"), y = 203, size = 5,
                          colour = "steelblue", fontface = "bold")
```
```{r}
indicador_boi |>
  #dplyr::filter(data >= "2020-01-01") |>
  ggplot2::ggplot(aes(x = data, y = a_vista_r)) +
              geom_line(color = "black") +
              geom_smooth(method = "loess", se = FALSE) +
              scale_x_date(date_breaks = "1 year", date_labels = "%Y-%m-%d") +
              theme(axis.text.x = element_text(size = 10, color = "black",
                angle = 60, hjust = 1, vjust = 1)) +
              theme_swd_line +
              labs(
                title = "Indicador @ do Boi Gordo",
                subtitle = "# Fonte: CEPEA/B3(2023)

                by Ítalo Monteiro",
                x = "Data"
              ) +
              geom_text(
                    data = indicador_boi[indicador_boi$data == "2023-08-25", ],
                            aes(label = scaleFUN(a_vista_r)),
                            hjust = -0.1,
                            vjust = -0.5,
                            size = 5,
                            color = "steelblue",
                            fontface = "bold") +
                annotate("text", label = "R$/@",
                    x = as.Date(
                        "2023-01-01"), y = 208, size = 5,
                          colour = "steelblue", fontface = "bold")
```