...
relacao_troca_nespro <- rt_nespro |>
  ...
  dplyr::group_by(ano, mes) |>
  dplyr::summarise(rt_medio = mean(rt, n)) |>
  dplyr::mutate(data = lubridate::ymd(paste(ano, mes, "01")), .before = rt_medio) 
```
...
colors <- "gray40"
title_text <- glue::glue("RELAÇÃO DE TROCA:<br>@ de boi vs 1 bezerro") # nolint
subtitle_text <- glue::glue("Quantas arrobas (@) de boi são necessarias para comprar um bezerro")
...
title_text1 <- glue::glue("RELAÇÃO DE TROCA:<br>kg de boi vs kg de bezerro") # nolint
subtitle_text1 <- glue::glue("Quantos kilos de boi gordo são necessarios<br>para compra de um kg de bezerro entre 6 e 12 meses")
...
```{r plot} 
plot_relacao_troca <- relacao_troca_nespro |>
  ggplot(aes(x = data, y = rt_medio)) +
  geom_col() +
  #geom_col(aes(fill = ifelse(data == max(data), "last",ifelse(data %in% c("2021-10-01", "2004-11-01", "2024-01-01"),"high", "default")))) +
  ...
  theme(
        ...
        axis.text.x = ggtext::element_markdown(
          face = "bold", family = font2 ,size = 8, color = "gray50", angle=60, hjust=1) 
  )
plot_relacao_troca
```
